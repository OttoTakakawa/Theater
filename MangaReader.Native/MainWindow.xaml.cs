...
using MangaReader.Native.Videos;
...
public partial class MainWindow : Window
{
    ...
    private void OpenVideoTest_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new System.Windows.Forms.OpenFileDialog
        {
            Title = "选择要播放的视频文件",
            Filter = "视频文件|*.mp4;*.mkv;*.avi;*.mov;*.wmv;*.webm;*.flv;*.m2ts;*.mpg;*.mpeg;*.m4v|所有文件|*.*",
            Multiselect = false
        };
        ...
        var videoId = ComputeVideoId(filePath);
        ...
        var db = new VideoDatabase(_storage.Root);
        db.Initialize();

        // Try to restore saved progress
        var saved = db.LoadVideo(videoId);
        if (saved != null)
        {
        ...
        var player = new PlayerWindow(video, db, _storage);
        player.Show();
    }

    private static string ComputeVideoId(string filePath)
    {
        using var sha256 = System.Security.Cryptography.SHA256.Create();
        var normalized = Path.GetFullPath(filePath).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar).ToLowerInvariant();
        var hash = sha256.ComputeHash(System.Text.Encoding.UTF8.GetBytes(normalized));
        return Convert.ToHexString(hash, 0, 8).ToLowerInvariant();
    }

    private void OpenCollectionTest_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new System.Windows.Forms.FolderBrowserDialog
        {
            Description = "选择包含图片和视频的合集文件夹"
        };
        ...
        var collection = new CollectionWindow(folderPath, db, _storage);
    ...
    private void NavVideo_Click(object sender, RoutedEventArgs e)
    {
        var videoLib = new VideoLibraryWindow(_storage);
        videoLib.Owner = this;
        videoLib.Show();
    }
