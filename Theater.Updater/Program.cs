...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            var extractRoot = Path.Combine(Path.GetTempPath(), "Theater_Update_" + Guid.NewGuid().ToString("N"));
        ...
        {
            var logPath = Path.Combine(Path.GetTempPath(), "Theater_Updater_Error.log");
    ...
    private static string ResolvePackageRoot(string packagePath, string extractRoot)
    {
        ...
        {
            var directoryExe = Path.Combine(packagePath, "Theater.exe");
        ...
        var directExe = Path.Combine(extractRoot, "Theater.exe");
        ...
        var nestedExe = Directory
            .EnumerateFiles(extractRoot, "Theater.exe", SearchOption.AllDirectories)
            .FirstOrDefault();
...
internal sealed record UpdateOptions(string PackagePath, string TargetDirectory, string ExecutableName, int ProcessId)
{
    public static UpdateOptions Parse(string[] args)
    {
        ...
        var executableName = values.TryGetValue("--exe", out var exe) ? exe : "Theater.exe";
