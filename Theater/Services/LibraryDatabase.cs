...
public sealed partial class LibraryDatabase
{
    ...
    private const string UpsertBookSql =
        ...
        INSERT INTO books(id, title, author, character_name, foreign_name, tags, produced_at, imported_at, summary,
                          folder_path, page_count, total_bytes, cover_page_index, last_read_page_index, is_missing, is_privacy_cover,
                          video_paths, image_set_paths, cover_image_path, duration_ms, last_position_ms, updated_at)
        VALUES ($id, $title, $author, $characterName, $foreignName, $tags, $producedAt, $importedAt, $summary,
                $folderPath, $pageCount, $totalBytes, $coverPageIndex, $lastReadPageIndex, $isMissing, $isPrivacyCover,
                $videoPaths, $imageSetPaths, $coverImagePath, $durationMs, $lastPositionMs, $updatedAt)
        ON CONFLICT(id) DO UPDATE SET
            ...
            image_set_paths = excluded.image_set_paths,
            cover_image_path = excluded.cover_image_path,
            duration_ms = CASE WHEN books.duration_ms = 0 THEN excluded.duration_ms ELSE books.duration_ms END,
    ...
    {
        // 检测并清理损坏的数据库文件（0 字节或过小，明显不是有效 SQLite 文件）。
        // SQLite 打开 0 字节文件时不会抛异常，但 WAL 初始化可能失败，
        // 导致后续 CREATE TABLE 静默不生效，最终引发 "no such table" 错误。
        EnsureDatabaseFileValid();
        ...
        EnsureColumn(connection, "books", "duration_ms", "INTEGER NOT NULL DEFAULT 0");
        EnsureColumn(connection, "books", "last_position_ms", "INTEGER NOT NULL DEFAULT 0");
        EnsureColumn(connection, "books", "video_paths", "TEXT NOT NULL DEFAULT '[]'");
        EnsureColumn(connection, "books", "image_set_paths", "TEXT NOT NULL DEFAULT '[]'");
        EnsureColumn(connection, "books", "cover_image_path", "TEXT NOT NULL DEFAULT ''");
        ...
        // last_opened_at 列通过 EnsureColumn 添加，索引必须在列存在后才能创建
        using var indexCmd = connection.CreateCommand();
        indexCmd.CommandText = "CREATE INDEX IF NOT EXISTS idx_books_last_opened_at ON books(last_opened_at);";
    ...
    /// <summary>
    /// 检查数据库文件是否有效。SQLite 数据库文件头部应该是
    /// "SQLite format 3\0"（16 字节）。如果文件存在但小于 16 字节或头部不匹配，
    /// 说明是损坏文件（常见于上次启动被强制中断、磁盘写一半等情况），
    /// 直接删除让后续 CREATE TABLE 能正常建库。
    /// </summary>
    private void EnsureDatabaseFileValid()
    {
        if (!File.Exists(_databasePath))
        {
            return;
        ...
        AppLogger.Warn(
            "database-init",
            $"检测到损坏的数据库文件（{info.Length} 字节），将删除后重建：{_databasePath}");
        ...
        try
        {
            ...
            {
                var companion = _databasePath + suffix;
        ...
        {
            AppLogger.Error("database-init", ex, "删除损坏数据库文件失败");
        }
    }
...
public sealed record BookmarkRecord(string BookId, int PageIndex, string CreatedAt);
    ...
    private static void AddBookParameters(SqliteCommand command, MangaBook book)
    {
        ...
        command.Parameters.AddWithValue("$isMissing", book.IsMissing ? 1 : 0);
        command.Parameters.AddWithValue("$isPrivacyCover", book.IsPrivacyCover ? 1 : 0);
        command.Parameters.AddWithValue("$videoPaths", book.ToVideoPathsJson());
        command.Parameters.AddWithValue("$imageSetPaths", book.ToImageSetPathsJson());
        command.Parameters.AddWithValue("$coverImagePath", book.CoverImagePath);
        command.Parameters.AddWithValue("$durationMs", book.DurationMs);
        command.Parameters.AddWithValue("$lastPositionMs", book.LastPositionMs);
        command.Parameters.AddWithValue("$updatedAt", DateTimeOffset.Now.ToString("O"));
    }
}
